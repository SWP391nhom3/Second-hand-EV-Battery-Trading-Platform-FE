...
const StaffDashboard = () => {
  ...
  const currentConstructFees = constructFees[selectedContract.construct_id] || { buyer: [], seller: [] };
  ...
  const buyerTotal = serviceFees
    ...
    .reduce((sum, f) => sum + calculateFeeAmount(f, selectedContract.vehicle.price), 0);
  
  const sellerTotal = serviceFees
    .filter(f => currentConstructFees.seller.includes(f.service_fee_id))
    .reduce((sum, f) => sum + calculateFeeAmount(f, selectedContract.vehicle.price), 0);
  
  const totalFees = serviceFees.reduce((sum, f) => sum + calculateFeeAmount(f, selectedContract.vehicle.price), 0);

  const pendingContracts = contracts.filter(c => c.status === 'pending_fee_selection').length;
        ...
        <div className={styles.statisticsGrid}>
          <div className={styles.statisticCard}>
            <div className={styles.statisticCardContent}>
              ...
              <div className={`${styles.statisticIconContainer} ${styles.iconBlue}`}>
          ...
          <div className={styles.statisticCard}>
            <div className={styles.statisticCardContent}>
              <div>
                ...
                <p className={`${styles.statisticValue} ${styles.statisticValueOrange}`}>{pendingContracts}</p>
              </div>
              <div className={`${styles.statisticIconContainer} ${styles.iconOrange}`}>
          ...
          <div className={styles.statisticCard}>
            <div className={styles.statisticCardContent}>
              <div>
                ...
                <p className={`${styles.statisticValue} ${styles.statisticValueGreen}`}>{completedContracts}</p>
              </div>
              <div className={`${styles.statisticIconContainer} ${styles.iconGreen}`}>
          ...
          <div className={styles.statisticCard}>
            <div className={styles.statisticCardContent}>
              <div>
                ...
                <p className={`${styles.statisticValue} ${styles.statisticValuePurple}`}>
                  {(contracts.reduce((sum, c) => sum + c.vehicle.price, 0) / 1000000000).toFixed(1)}B
                </p>
              </div>
              <div className={`${styles.statisticIconContainer} ${styles.iconPurple}`}>
                ...
                {filteredContracts.map(contract => (
                  <div
                    ...
                    className={`${styles.contractListItem} ${
                      selectedContract.construct_id === contract.construct_id ? styles.contractListItemActive : ''
                    }`}
                  >
                    <div className={styles.contractListItemHeader}>
                      <div>
                        <p className={styles.contractId}>{contract.construct_id}</p>
                        <p className={styles.vehicleName}>{contract.vehicle.name}</p>
                        <p className={styles.licensePlate}>{contract.vehicle.licensePlate}</p>
                      </div>
                    ...
                    <div className={styles.contractListItemFooter}>
                      ...
                      <span className={styles.createdAt}>{contract.created_at}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className={styles.contractDetailsColumn}>
            ...
            <div className={styles.contractInfoBox}>
              ...
              <div className={styles.vehicleInfoGrid}>
                <div className={styles.infoCard}>
                  ...
                  <p className={styles.infoCardTextBold}>{selectedContract.vehicle.name}</p>
                  <p className={styles.infoCardTextSmall}>Năm: {selectedContract.vehicle.year}</p>
                  <p className={styles.infoCardTextSmall}>Biển số: {selectedContract.vehicle.licensePlate}</p>
                ...
                <div className={styles.infoCard}>
                  <div className={styles.buyerSellerCardHeader}>
                    <div className={styles.buyerSellerCardTitleContainer}>
                      ...
                      <span className={styles.buyerSellerCardTitle}>Người Mua</span>
                    </div>
                  ...
                  <p className={styles.infoCardTextBold}>{selectedContract.buyer.full_name}</p>
                  <p className={styles.infoCardTextSmall}>{selectedContract.buyer.phone}</p>
                  <p className={`${styles.infoCardTextSmall} truncate`}>{selectedContract.buyer.email}</p>
                </div>
                ...
                <div className={styles.infoCard}>
                  <div className={styles.buyerSellerCardHeader}>
                    <div className={styles.buyerSellerCardTitleContainer}>
                      ...
                      <span className={styles.buyerSellerCardTitle}>Người Bán</span>
                    </div>
                    <div className={styles.actionButtons}>
                      <button
                  ...
                  <p className={styles.infoCardTextBold}>{selectedContract.seller.full_name}</p>
                  <p className={styles.infoCardTextSmall}>{selectedContract.seller.phone}</p>
                  <p className={`${styles.infoCardTextSmall} truncate`}>{selectedContract.seller.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className={styles.feeDistributionTableContainer}>
              <div className={styles.feeDistributionTableHeader}>
                <h2 className={styles.feeDistributionTableTitle}>Bảng Phân Bổ Chi Phí</h2>
              </div>
                ...
                <table className={styles.feeTable}>
                  <thead className={styles.feeTableHeader}>
                    <tr>
                      ...
                      <th className={`${styles.feeTableHeaderTh} ${styles.feeTableHeaderThCenter}`}>Người Mua</th>
                      <th className={`${styles.feeTableHeaderTh} ${styles.feeTableHeaderThCenter}`}>Người Bán</th>
                    </tr>
                  </thead>
                  <tbody className={styles.feeTableBody}>
                    {serviceFees.map(fee => {
                      const feeAmount = calculateFeeAmount(fee, selectedContract.vehicle.price);
                      const Icon = fee.icon;
                      return (
                        <tr key={fee.service_fee_id} className={styles.feeTableRow}>
                          ...
                          <td className={`${styles.feeTableCell} ${styles.feeAmount}`}>
                            {feeAmount.toLocaleString('vi-VN')} ₫
                          </td>
                          <td className={styles.feeTableCell}>
                            {currentConstructFees.buyer.includes(fee.service_fee_id) ? (
                          ...
                          <td className={styles.feeTableCell}>
                            {currentConstructFees.seller.includes(fee.service_fee_id) ? (
                              <div className={styles.checkIconContainer}>
                                <Check className={styles.checkIcon} />
                              </div>
                            ) : (
                  ...
                  <tfoot className={styles.feeTableFooter}>
                    <tr>
                      ...
                      <td className={`${styles.feeTableFooterCell} ${styles.feeTableFooterCellCenterGreen}`}>
                        {buyerTotal.toLocaleString('vi-VN')} ₫
                      </td>
                      <td className={`${styles.feeTableFooterCell} ${styles.feeTableFooterCellCenterPurple}`}>
            ...
            <div className={styles.paymentSummaryGrid}>
              <div className={styles.paymentSummaryCardGreen}>
                <div className={styles.paymentSummaryHeader}>
                  <h3 className={styles.paymentSummaryTitle}>Người Mua Thanh Toán</h3>
                  <DollarSign className={styles.paymentSummaryIcon} />
                </div>
                <p className={styles.paymentSummaryAmount}>{buyerTotal.toLocaleString('vi-VN')} ₫</p>
                <p className={styles.paymentSummaryFeeCountGreen}>{currentConstructFees.buyer.length} khoản phí đã chọn</p>
              </div>

              <div className={styles.paymentSummaryCardPurple}>
                <div className={styles.paymentSummaryHeader}>
                  <h3 className={styles.paymentSummaryTitle}>Người Bán Thanh Toán</h3>
                  <DollarSign className={styles.paymentSummaryIcon} />
                </div>
                <p className={styles.paymentSummaryAmount}>{sellerTotal.toLocaleString('vi-VN')} ₫</p>
                <p className={styles.paymentSummaryFeeCountPurple}>{currentConstructFees.seller.length} khoản phí đã chọn</p>
              </div>
            </div>
